...
function Rotation.Active()
    if DMW.Settings.profile.HUD.Rotation == 1 and not DMW.Player.ReallyDeadPlayer and (not DMW.Helpers.Rotation.CastingCheck or not DMW.Player.Casting) and not IsMounted() and not DMW.Player.NoControl and (not DMW.Helpers.Rotation.StandingCheck or DMW.Player:Standing()) then
        return true
    end
    return false
end
